...
** Added
- In-flight fetch indicator: while completion data loads (for example the org repo list at ~/github:OWNER/~), a ~[fetching...]~ marker appears in the minibuffer. It is drawn as minibuffer text, so it works with any completion UI (vertico, icomplete, default). Toggle with ~remoto-show-fetch-indicator~ (default t).
- ~remoto-show-fetch-indicator~ - customization to enable or disable the indicator.
...
- Unit tests covering the in-flight counter (single decrement on success, error, and synchronous failure; correct behavior with overlapping fetches), the indicator overlay lifecycle, and the completion-refresh invalidation.
