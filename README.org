...
Requires [[https://cli.github.com][gh CLI]] installed and authenticated (~gh auth login~).
...
Install it in =~/.doom.d/packages.el=:

#+begin_src emacs-lisp
(package! remoto :recipe (:host github :repo "agzam/remoto.el"))
#+end_src

Then load it in =~/.doom.d/config.el=:
