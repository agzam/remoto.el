...
The mode turns itself on at first use, whichever way that comes: ~M-x remoto-browse~, an Embark open action, or a ~/github:~ or ~/gh:~ path typed at ~C-x C-f~, ~C-x d~ or any other file prompt. For the last one the package autoloads register a small handler for those two prefixes, the way TRAMP loads on the first remote path; it hands over to the real handler and steps aside. Each of these says so once in the echo area. To have the mode on from the start, and skip that message, put ~(global-remoto-mode 1)~ in your config.

The package is not on MELPA yet. Maybe upvote the [[https://github.com/melpa/melpa/pull/10219][submission PR]], who knows, perhaps it gets accepted faster.
...
Each level shows annotations: repo descriptions, issue/PR titles and state, file commit messages, user/org type. Works with vanilla completion, Vertico, Corfu, etc. While a GitHub request is in flight, a ~[fetching...]~ label sits at the end of the minibuffer input, and once the path is chosen and opens, the same label sits in the echo area; ~remoto-browse~ shows the same two, and ~remoto-show-fetch-indicator~ turns both off.
...
| ~remoto-copy-url~    | Copy ~github.com~ URL for current file/line to kill ring      |
