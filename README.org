...
Loading the package defines things and changes nothing. ~global-remoto-mode~ is the switch: it installs the file handler, the URL rewriting in ~find-file~ and ~dired~, and the completion metadata, and removes them all when turned off. Turn it on in your config:

#+begin_src emacs-lisp
(use-package remoto
  :straight (:host github :repo "agzam/remoto.el")
  :demand t
  :config (global-remoto-mode 1))
#+end_src

~remoto-browse~ and the Embark open actions turn the mode on themselves, so ~M-x remoto-browse~ works without that line. A ~/gh:~ path typed into ~C-x C-f~ before anything else needs it.
...
With ~global-remoto-mode~ on, GitHub URLs are detected automatically in ~dired~ and ~find-file~:
